    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (command.equals("SHOW")) {
            ...
            if (visualizerType.equals("Chromatogram")) {
                ...
                List <Feature> peaks = new ArrayList <> ();
                
                try {
                    int peakListHashCode = Integer.parseInt(peakIdentity
                            .getPropertyValue("FROM_PEAKLIST_HASHCODE"));
                    
                    PeakList peakList = null;
                    for (final PeakList list : 
                            MZmineCore.getProjectManager()
                                    .getCurrentProject().getPeakLists())
                    {
                        if (list.hashCode() == peakListHashCode)
                            peakList = list;
                    }
                    
                    String strPeakIndices = 
                            peakIdentity.getPropertyValue("ADAP_PEAK_IDS");
                    
                    for (final String pair : strPeakIndices
                            .substring(1, strPeakIndices.length() - 1)
                            .split(","))
                    {
                        final String[] strEntry = pair.split("=");
                        final double mz = Double.parseDouble(strEntry[0]);
                        final int id = Integer.parseInt(strEntry[1]);
                        
                        peaks.add(new SimpleFeature(
                                peakList.getRow(id).getBestPeak()));
                    }
                    
                    TICVisualizerModule.showNewTICVisualizerWindow(dataFiles,
                            peaks.toArray(new Feature[peaks.size()]), 
                            labelMap, 
                            scanSelection, //new ScanSelection(bestPeak.getRawDataPointsRTRange(), 1),
                ...
                for (int i = 0; i < selectedPeaks.length; ++i) {
                    ...
                    if (ip != null && ip.getDescription() == "Spectrum") 
                    {
                        // Multiply spectrum by -1
                        for (DataPoint dataPoint : ip.getDataPoints())
                            ((SimpleDataPoint) dataPoint).setIntensity(
